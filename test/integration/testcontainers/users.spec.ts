...
describe('UsersService (integration)', () => {
  ...
  it('logs in a user and returns tokens generated by the JWT service', async () => {
    ...
    expect(redisServiceMock.raw.sAdd).toHaveBeenCalledWith(expect.stringMatching(/^user_sessions:/), expect.any(String));
  });
  ...
  it('resets the password, clears the token from redis, and notifies the user', async () => {
    ...
    redisServiceMock.raw.getDel.mockResolvedValue(rawPayload);
    ...
    expect(redisServiceMock.raw.getDel).toHaveBeenCalledWith('reset_password_user:erin@example.com');
